...
Kaiseki.prototype = {
  ...
  getObject: function(className, objectId, params, callback) {
    ...
    this._jsonRequest('GET', url, params, false, callback);
  },
  ...
  deleteObject: function(className, objectId, callback) {
    var url = '/1/classes/' + className + '/' + objectId;
    this._jsonRequest('DELETE', url, null, false, callback);
  },

  getObjects: function(className, params, callback) {
    var url = '/1/classes/' + className;
    this._jsonRequest('GET', url, params, true, callback);
  },
  ...
  _jsonRequest: function(method, url, params, expectArray, callback) {
    if (_.isFunction(params)) {
      callback = params;
      params = null;
    } else {
    ...
    var wrap = function(err, res, body) {
      if (!err)
        body = expectArray ? JSON.parse(body).results : JSON.parse(body);
      callback(err, res, body);
    };
    this.request(method, url, params, wrap);
  }
};
